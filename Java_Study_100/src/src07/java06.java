...
public class java06 {
	...
	public static void main(String[] args) {
		// [1] : 객체 생성
		Student1 s1 = new Student1();	// 지극히 정상 --> 에러날 이유가 하나도 없다.
		
		// [2] : 객체 생성 --> 타입은 부모 타입으로 생성
		Person1 s2 = new Person1();  	// 이것도 정상 --> 하위(자식) 클래스로 객체를 만들면서 타입은 상위(부모) 타입을 쓰는 것이 가능 --> 다형성
		
		// [3] : 객체 생성
		Person1 aaa = new Person1();	// 지극히 정상
